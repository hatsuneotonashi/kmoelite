...
| 平台 | 状态 |
| --- | --- |
| iPhone | 开发预览可用；iPhone simulator 已通过 packaged debug app 安装、启动、首屏渲染、session restore 和 debug 详情页路由/视觉 smoke；iOS deep link 已注册并加固 pending route 交付，Reader/下载、签名真机、文件导出/分享仍需继续补齐。 |
...
| Apple TV | 原生 tvOS 开发预览工程已存在；`pnpm test:appletv` 和 `pnpm smoke:appletv-sim` 已通过 Apple TV 4K simulator 构建、安装、启动和截图解码。当前只是最薄 SwiftUI/tvOS shell 与 parser/SQLite 基础，真实 Reader、EPUB 获取、缓存窗口、遥控器 Reader 和实体设备/签名发布仍未完成；tvOS SDK 不提供 WebKit，不能复用当前 Tauri/WKWebView 前端壳。 |

当前状态和限制见 [docs/status](docs/status/README.md) 与 [docs/platforms](docs/platforms/README.md)。

## 最近更新

- 2026-06-18：Android live Reader smoke 现在可用 `pnpm smoke:android-live-reader` 复现登录、详情、EPUB 下载、Reader 打开和翻页；EPUB/MOBI 无真实大小的元数据条目不会再被当成可下载入口。
- 2026-06-18：新增独立原生 Apple TV SwiftUI/tvOS 开发预览工程，并提供 `pnpm test:appletv` 与 `pnpm smoke:appletv-sim`。
- 2026-06-18：Reader 滚动窗口清理现在会带走窗口外的 failed/missing 阅读缓存记录，但不会动正在准备或清理中的章节。
- 2026-06-18：Reader cache 修复现在可用同卷的另一种阅读归档兜底，例如源图 ZIP cache 失效时可用本地 EPUB 重新准备阅读缓存。
- 2026-06-18：资料库中的 MOBI 文件保持文件-only，即使同卷已有 Reader cache 也不会被误显示为内置阅读入口。
...
- Apple TV：已经新增独立原生 SwiftUI/tvOS 工程，不走 Tauri/WKWebView；当前只完成开发预览 shell、基础站点解析/SQLite 测试和 simulator 启动 smoke。后续仍要补真实登录、EPUB 获取、横屏 Reader、遥控器翻页、前一章/当前章/后一章缓存窗口、显式删除本地阅读数据、实体设备和签名发布。
