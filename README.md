...
| Android 手机 | 实验预览源码路径存在；Tauri Android debug APK/AAB 可构建，Pixel 8 模拟器可启动手机布局；真机、下载、Reader 和签名发布仍未完整验证。 |
| Android 平板 | 实验预览源码路径存在；Android 平板模拟器可启动 tablet contract，并通过一次真实 app 登录 smoke；下载、Reader、缓存清理、真机和签名发布仍未完整验证。 |
...
- 2026-06-17：新增 Android TV 实验入口，验证 Leanback launcher、TV runtime 识别、宽屏 shell 和方向键焦点 smoke。
- 2026-06-17：新增 Android Tauri 工程与 debug APK/AAB 构建路径，并修复 Android phone 被误判为 Linux 桌面布局的问题。
...
- Android 手机/平板：已有调试 APK 路径、手机/平板布局 smoke 和 Android 平板真实 app 登录 smoke，可作为后续 Android 适配基础；当前仍不建议当作稳定日常版本。
...
- Apple TV：后续研究遥控器输入、横屏 Reader、焦点导航和缓存策略，不承诺时间表。
