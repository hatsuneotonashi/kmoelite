...
| 平台 | 状态 |
| --- | --- |
| iPhone | 开发预览可用；iPhone simulator 已通过 packaged debug app 安装、启动、首屏渲染、session restore 和 `kmoelite://comic/<id>` open-url smoke，详情/Reader/下载、签名真机、文件导出/分享仍需继续补齐。 |
...
- 2026-06-18：iOS packaged build 注册 `kmoelite://comic/<id>` 深链路；iPhone simulator 安装、启动和 open-url smoke 通过。
- 2026-06-18：Android 系统分享导出桥补齐失败回传；Rust 路径/SQLite 校验通过后才调用系统分享，原生 bridge 无法打开时会返回明确错误。
