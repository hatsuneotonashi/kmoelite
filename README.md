...
| 平台 | 状态 |
| --- | --- |
| iPhone | 开发预览可用；iPhone simulator 已通过 packaged debug app 安装、启动、首屏渲染和 session restore smoke，详情/Reader/下载、签名真机、文件导出/分享仍需继续补齐。 |
...
| Android 手机 | 实验预览源码路径存在；Pixel 8 模拟器已通过真实登录、详情、EPUB 下载、Reader、翻页和本地阅读数据删除 smoke；系统分享桥源码和 debug build 已通过，真机、分享运行时 smoke 和签名发布仍未完整验证。 |
| Android 平板 | 实验预览源码路径存在；Pixel Tablet 模拟器已通过真实登录、详情、EPUB 下载、Reader、双页翻页和本地阅读数据删除 smoke；系统分享桥源码和 debug build 已通过，真机、分享运行时 smoke 和签名发布仍未完整验证。 |
| Android TV | 实验预览入口存在；Android TV 模拟器已通过真实登录、详情、EPUB 下载、Reader、遥控器翻页和本地阅读数据删除 smoke，系统分享桥源码和 debug build 已通过；实体 TV、分享运行时 smoke 和签名发布仍未完成。 |
| Apple TV | 后续研究方向；本机已安装 tvOS simulator runtime，可创建并启动 Apple TV 模拟器；仍缺可运行 tvOS 壳、遥控器输入和平台分发验证。 |

当前状态和限制见 [docs/status](docs/status/README.md) 与 [docs/platforms](docs/platforms/README.md)。

## 最近更新

- 2026-06-17：Android 增加系统分享导出桥；Rust 路径/SQLite 校验通过后，可把 App 私有目录里的本地文件交给系统分享。
- 2026-06-17：修复已恢复站点会话但首页仍显示“登录账号”的误判；iPhone simulator packaged app 已确认首页会显示“账号中心”。
- 2026-06-17：应用可见名称统一为 kmoelite；iOS simulator packaged app 已生成并安装启动为 `kmoelite.app`，旧 `Kmoe Client.app` 构建残留会自动清理。
- 2026-06-17：修复 Android 手机封面取色详情页目录区域文字/背景断层问题；Android phone、tablet、TV 模拟器完成 packaged 启动和主界面 smoke。
- 2026-06-17：iPad simulator 已通过真实 EPUB 下载到 Reader、翻页和进度写入 smoke；iPhone simulator 已通过 packaged render 和 session restore smoke。
...
- Apple TV：平台检查已经能暴露 tvOS 工具链状态，本机可启动 Apple TV 模拟器；后续仍需实现可运行 tvOS 壳、遥控器输入、横屏 Reader、焦点导航和缓存策略，不承诺时间表。
