...
| Android TV | 实验预览入口存在；Leanback launcher、TV runtime 识别、方向键焦点、remote Back、native DPAD/OK 输入桥和合成本地 Reader cache OK/Back smoke 已验证，下载/缓存清理/实体 TV 全链路仍未完成。 |
...
- 2026-06-17：补齐 Android TV native DPAD/OK 输入桥，并在 TV 模拟器用合成本地 Reader cache 验证 OK/Back smoke。
- 2026-06-17：修复 native 登录会话验收误判，Android 平板模拟器已通过真实 app 登录 smoke。
...
- Android TV：已有实验启动、方向键焦点、native remote 输入桥和合成本地 Reader OK/Back smoke；下一步仍需补下载、缓存清理和真设备验证。
