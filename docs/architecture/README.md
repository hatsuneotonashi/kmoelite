...
临时 Reader cache 可以按滚动章节窗口和容量策略自动清理；自动清理不能删除永久下载、书架记录、阅读历史或队列记录。用户显式“删除本地阅读数据”可删除对应 Reader cache、Reader-capable `source_zip` / `epub` 源文件、资料库记录和终态源文件任务，但仍必须保留书架、阅读进度和阅读历史。永久下载和 Library records 必须代表明确用户意图，不应成为普通在线阅读的默认路径。

## 存储策略

- 普通阅读优先准备临时 Reader cache。
- 默认保留前一章、当前章和后一章；章节切换后，窗口外旧缓存应优先清理。
- 当策略保留后一章时，可从可信本地 archive 预取下一章；当策略不保留后一章时，应跳过预取。
- 显式下载、永久文件和显示位置功能保留为高级/兼容能力。
- iPhone/iPad 的显式下载先落在 App 私有保存区，完成后再通过系统分享/导出交给 Files 或其他目标。
