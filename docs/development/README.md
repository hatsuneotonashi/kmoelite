...
`pnpm smoke:ios-sim` 会优先选择已启动的 iOS simulator；如果同时启动了 Apple TV simulator，也不会把 iOS app 误装到 tvOS。启动后脚本会截一张临时 simulator 截图并确认系统可解码，截图自动删除、不进入仓库。设置 `IOS_SIM_COMIC_ID` 时，脚本会额外打开安全的 `kmoelite://comic/<id>` deep link；这只验证 URL scheme/open-url 入口，不等同于已自动点掉 iOS 系统确认框或完成详情页视觉验证。不要用裸 `xcodebuild` 替代 Tauri iOS 构建脚本；它缺少 Tauri mobile RPC 上下文。打包入口也必须保持相对资源路径，不要在 `index.html` 写根 `<base href="/">`。
