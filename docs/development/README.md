...
- Apple TV：未来方向；`check:platforms` 已检查 tvOS SDK、Apple TV simulator device type、实际 simulator device、tvOS simulator runtime 和 tvOS Rust targets；本机可启动 Apple TV 模拟器，但仍需要单独实现 tvOS 壳、遥控器输入、焦点导航和横屏 Reader。
