...
- Android phone/tablet have an experimental Tauri Android source/build path and phone emulator launch smoke. Android TV has an experimental Leanback launcher/runtime-detection path, emulator launch/focus smoke, native DPAD/OK input bridging, remote Back handling, and synthetic local Reader-cache OK/Back smoke. These remain incomplete until real device, download, Reader cache cleanup, and signed-release validation are finished. Apple TV remains a research target.
