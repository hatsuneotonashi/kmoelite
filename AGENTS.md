...
- Settings full Reader cache cleanup must call the native `clear_reading_cache` command when available and must also clear every frontend reading-cache row, including failed, missing, and legacy `reading` rows. Browser-preview cleanup is fallback only.
