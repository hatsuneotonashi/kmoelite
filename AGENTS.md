...
- Android phone/tablet have an experimental Tauri Android source/build path, phone/tablet emulator launch smoke, and one Android tablet emulator live app login smoke. Android TV has an experimental Leanback launcher/runtime-detection path, emulator launch/focus smoke, native DPAD/OK input bridging, remote Back handling, and synthetic local Reader-cache OK/Back smoke. These remain incomplete until real device, download, Reader cache cleanup, and signed-release validation are finished. Apple TV remains a research target.
...
- Planned target platforms are iPhone, iPad, Android phone, Android tablet, Windows, and macOS. Android phone/tablet currently have an experimental Tauri Android project and debug build path, not stable release status. Android TV currently has only the thinnest experimental Android/Leanback shell, remote-focus baseline, native DPAD/OK input bridge, remote Back navigation, and synthetic local Reader-cache OK/Back smoke. Apple TV remains a future research target until platform architecture and input constraints are designed.
