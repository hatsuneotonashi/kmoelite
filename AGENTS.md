...
- Android phone/tablet have an experimental Tauri Android source/build path and emulator smoke coverage through live EPUB Reader flows. Android TV has an experimental Leanback launcher/runtime-detection path, emulator launch/focus smoke, native DPAD/OK input bridging, remote Back handling, and emulator smoke coverage through live EPUB Reader flows. These remain incomplete until real device, file export/share, and signed-release validation are finished. Apple TV now has a separate native SwiftUI/tvOS developer-preview project under `apps/kmoe-appletv`; it is not a Tauri/WKWebView shell. Current tvOS SDKs do not provide WebKit, so Apple TV work must stay native tvOS until a different viable platform strategy is proven.
...
- Planned target platforms are iPhone, iPad, Android phone, Android tablet, Windows, and macOS. Android phone/tablet currently have an experimental Tauri Android project and debug build path, not stable release status. Android TV currently has an experimental Android/Leanback shell, remote-focus baseline, native DPAD/OK input bridge, remote Back navigation, and emulator smoke coverage through live EPUB Reader flows; it still needs real-device and signed-release validation. Apple TV has a separate native SwiftUI/tvOS developer-preview project with simulator build/install/launch smoke and parser/SQLite unit tests, but it is not Reader-complete and still needs real login, EPUB fetch, remote Reader controls, cache cleanup, device signing, and distribution validation. Do not claim an Apple TV WKWebView/Tauri shell: current tvOS SDKs do not ship WebKit, so Apple TV code must stay native tvOS unless a different viable platform strategy is proven.
...
- Guarded real-download validation scripts may process real ordinary queue items, including Reader archive tasks, to prove the full app path. They must require an explicit live/real-download confirmation env var, keep authorization URLs in memory, use a temporary download directory by default, clean it up unless the operator explicitly opts to keep it, and print only redacted summaries without local paths.
...
- Automatic policy cleanup may include ready, failed, and missing reading-cache rows outside the retention window, but it must not evict preparing or evicting in-flight rows.
