...
export function readableAppMessage(input: unknown, fallback = '操作暂时无法完成，请稍后重试。'): string {
  ...
  if (/(Documents|Downloads)\/Kmoe/.test(normalized)) return '文件已保存在 App 内，可用“导出文件”保存到“文件”App 或其他位置。'
  ...
  if (/登录状态已失效|未登录|登录已过期|not authenticated|401/i.test(normalized)) return '登录状态已失效，请重新登录。'
  if (/未能取得可用的下载地址|站点拒绝.*下载|下载响应不是文件内容|确认登录状态和下载权限|剩余额度|权限和额度|没有下载权限|沒有下載權限|权限不足|權限不足|额度不足|額度不足|insufficient quota|VIP only|no permission|forbidden|403/i.test(normalized)) {
    return '站点没有返回可下载文件，请重新登录并检查账号额度或下载权限。'
  }
  if (/无法写入下载文件|无法保存下载文件|无法创建下载目录|download directory|failed to create mobile app download directory/i.test(normalized)) {
    return '无法写入保存位置，请检查设备剩余空间后重试。'
  }
...
export function readableDownloadIssue(input?: string): string {
  ...
  if (!message) return ''
  if (/登录状态已失效|未登录|登录已过期|not authenticated|401/i.test(message)) return '登录状态已失效，请重新登录后重试。'
  if (/未能取得可用的下载地址|站点拒绝.*下载|下载响应不是文件内容|确认登录状态和下载权限|剩余额度|权限和额度|没有下载权限|沒有下載權限|权限不足|權限不足|额度不足|額度不足|insufficient quota|VIP only|no permission|forbidden|403/i.test(message)) {
    return '站点没有返回可下载文件，请重新登录并检查账号额度或下载权限。'
  }
  if (/无法写入下载文件|无法保存下载文件|无法创建下载目录|failed to create mobile app download directory/i.test(message)) {
    return '无法写入保存位置，请检查设备剩余空间后重试。'
  }
