...
export function readableAppMessage(input: unknown, fallback = '操作暂时无法完成，请稍后重试。'): string {
  ...
  if (/未能取得可用的下载地址|站点拒绝.*下载|下载响应不是文件内容|确认登录状态和下载权限|剩余额度|权限和额度|forbidden|403/i.test(normalized)) {
...
export function readableDownloadIssue(input?: string): string {
  ...
  if (/未能取得可用的下载地址|站点拒绝.*下载|下载响应不是文件内容|确认登录状态和下载权限|剩余额度|权限和额度|forbidden|403/i.test(message)) {
    return '站点没有返回可下载文件，请重新登录并检查账号额度或下载权限。'
  }
