...
- 行为摘要：使用已存在的 `Kmoelite_Tablet_API_36` Android tablet emulator 复跑 `pnpm smoke:android-device`，验证通用 Android packaged smoke 入口同样能覆盖 tablet 形态的 debug APK build、adb install、启动、运行中 `kmoelite://comic/<id>` deep link 和临时截图解码。
- 验证：
  - `emulator -list-avds`：passed，存在 phone/tablet/TV AVD。
  - `emulator -avd Kmoelite_Tablet_API_36 -no-snapshot-save -no-boot-anim -gpu swiftshader_indirect -no-audio -no-window`：started。
  - `adb wait-for-device` + `adb shell getprop sys.boot_completed`：passed，设备 `emulator-5554` 进入 `device` 状态。
  - `ANDROID_DEVICE_ID=emulator-5554 ANDROID_RENDER_WAIT_SECONDS=1 ANDROID_COMIC_ID=10817 pnpm smoke:android-device`：passed；完成 frontend build、Tauri Android debug APK/AAB build、adb install、app launch、safe comic deep link 和临时截图解码，输出 `screenshot=2560x1600`；截图位于系统临时目录并由脚本删除。
  - `adb -s emulator-5554 emu kill`：passed，验证后关闭 emulator。
  - `git diff --check`：passed。
  - `pnpm check:platforms`：passed，`pass=58 warn=1 external=2 fail=0`。
  - `pnpm --dir apps/kmoe-app typecheck`：passed。
  ...
  - `cargo test --manifest-path apps/kmoe-app/src-tauri/Cargo.toml --lib`：passed，92 tests。
  - `node scripts/check-ios-assets.mjs`：passed，files=27。
- 未运行项：未运行 Playwright E2E；本轮未修改生产代码、路由、布局、Reader、accessibility、视觉基线或浏览器可见工作流。未运行真实站点 smoke、真实下载验证、Android 真机、Android TV emulator 复跑、Windows 真机、iPhone/iPad 真机或 macOS 签名/公证验证。
- 待发布风险：该结果证明 Android tablet emulator 可通过通用 adb smoke 入口安装、启动、deep link 和截图；不代表 Android tablet 真机、真实文件分享、签名 release 或分发验证完成。

## 2026-06-18 Android phone emulator adb smoke 复跑

- 变更范围：TASK_PROGRESS、`docs/status/README.md`、CHANGELOG。
- 行为摘要：使用已存在的 `Pixel_8_API_36` Android phone emulator 复跑 `pnpm smoke:android-device`，验证上轮新增脚本不只是入口存在，而是能完成真实 debug APK build、adb install、启动、运行中 `kmoelite://comic/<id>` deep link 和临时截图解码。
- 验证：
  - `emulator -list-avds`：passed，存在 phone/tablet/TV AVD。
  - `emulator -avd Pixel_8_API_36 -no-snapshot-save -no-boot-anim -gpu swiftshader_indirect -no-audio -no-window`：started。
  - `adb wait-for-device` + `adb shell getprop sys.boot_completed`：passed，设备 `emulator-5554` 进入 `device` 状态。
  - `ANDROID_DEVICE_ID=emulator-5554 ANDROID_RENDER_WAIT_SECONDS=1 ANDROID_COMIC_ID=10817 pnpm smoke:android-device`：passed；完成 frontend build、Tauri Android debug APK/AAB build、adb install、app launch、safe comic deep link 和临时截图解码，输出 `screenshot=1080x2400`；截图位于系统临时目录并由脚本删除。
  - `adb -s emulator-5554 emu kill`：passed，验证后关闭 emulator。
  - `git diff --check`：passed。
  - `pnpm check:platforms`：passed，`pass=58 warn=1 external=2 fail=0`。
  - `pnpm --dir apps/kmoe-app typecheck`：passed。
  ...
  - `cargo test --manifest-path apps/kmoe-app/src-tauri/Cargo.toml --lib`：passed，92 tests。
  - `node scripts/check-ios-assets.mjs`：passed，files=27。
- 未运行项：未运行 Playwright E2E；本轮未修改生产代码、路由、布局、Reader、accessibility、视觉基线或浏览器可见工作流。未运行真实站点 smoke、真实下载验证、Android 真机、Android tablet/TV emulator 复跑、Windows 真机、iPhone/iPad 真机或 macOS 签名/公证验证。
- 待发布风险：该结果证明 Android phone emulator 可通过通用 adb smoke 入口安装、启动、deep link 和截图；不代表 Android 真机、Android tablet/TV、真实文件分享、签名 release 或分发验证完成。
...
- 行为摘要：显式下载格式选择器和格式校验顺序统一为 EPUB、源图 ZIP、MOBI；普通下载/阅读入口优先展示 Reader-capable EPUB，源图 ZIP 保留为高级高画质选项，MOBI 保留为用户手动选择的文件格式。
- 验证：
  - `pnpm --dir apps/kmoe-app exec vitest run src/tests/settingsNativeConfig.test.tsx src/tests/detailReaderEntry.test.tsx`：passed，2 files / 25 tests。第一次运行发现测试未展开详情页离线下载面板，修正测试定位后复跑通过。
  - `git diff --check`：passed。
  - `pnpm --dir apps/kmoe-app typecheck`：passed。
  - `pnpm --dir apps/kmoe-app test:run`：passed，55 files / 315 tests。
  - `pnpm --dir apps/kmoe-app build`：passed，production Vite build and iOS asset sync completed；生成产物保持 ignored。
...
- 待发布风险：该修复只调整显式格式选择顺序；真实平台下载、导出/分享、签名发布和真机验证仍按平台文档继续验证。
...
- 行为摘要：只有迁移元数据、没有真实本地 EPUB/source ZIP 文件或 Reader cache 的资料库记录，不再被当成“本地阅读数据存在”。相关删除入口不会再对这种记录展示假清理路径；需要阅读时仍由在线获取/准备流程重新生成真实本地数据。
- 验证：
  - `git diff --check`：passed。
  - `pnpm --dir apps/kmoe-app exec vitest run src/tests/localReadingData.test.ts`：passed，1 file / 2 tests。
  - `pnpm --dir apps/kmoe-app typecheck`：passed。
  - `pnpm --dir apps/kmoe-app test:run`：passed，55 files / 311 tests。
  - `pnpm --dir apps/kmoe-app build`：passed，production Vite build and iOS asset sync completed；生成产物保持 ignored。
  - `cargo fmt --all --manifest-path apps/kmoe-app/src-tauri/Cargo.toml -- --check`：passed。
  - `cargo check --manifest-path apps/kmoe-app/src-tauri/Cargo.toml`：passed。
  - `cargo test --manifest-path apps/kmoe-app/src-tauri/Cargo.toml --lib`：passed，91 tests。
...
- 未运行项：未运行真实下载验证、iPhone/iPad 真机、Android 真机/TV 实体设备或 Windows 真机。
- 待发布风险：该修复只收紧 metadata-only Library 记录的本地数据存在性判断；各平台真实下载、导出/分享、签名发布和真机验证仍按平台文档继续验证。
...
- 行为摘要：Rust native 下载队列启动后如果没有 claim 到任何待处理任务，现在返回明确错误“没有待处理下载任务”，不再把直接启动空队列误报为成功。
- 验证：
  - `git diff --check`：passed。
  - `pnpm --dir apps/kmoe-app typecheck`：passed。
  - `pnpm --dir apps/kmoe-app test:run`：passed，55 files / 309 tests。
  - `pnpm --dir apps/kmoe-app build`：passed，production Vite build and iOS asset sync completed；生成产物保持 ignored。
  - `cargo fmt --all --manifest-path apps/kmoe-app/src-tauri/Cargo.toml -- --check`：passed。
  - `cargo check --manifest-path apps/kmoe-app/src-tauri/Cargo.toml`：passed。
  - `cargo test --manifest-path apps/kmoe-app/src-tauri/Cargo.toml --lib`：passed，91 tests。
  ...
  - `node scripts/check-ios-assets.mjs`：passed，files=27。
  - 敏感扫描：passed；唯一命中是 `scripts/verify-release-readiness.sh` 中用于检测敏感信息的正则规则文本，未发现真实账号、密码、Cookie、Session、Token、授权 URL 或本机私有路径。
- 未运行项：未运行 Playwright E2E；本轮只改 Rust 下载队列空启动边界和对应测试，没有改路由、布局、Reader、accessibility、视觉基线或浏览器可见工作流。未运行真实下载验证、iPhone/iPad 真机、Android 真机/TV 实体设备或 Windows 真机。
